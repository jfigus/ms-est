...
EXPORT EST_ERROR est_init_logger(EST_LOG_LEVEL lvl, void (*loggerfunc)(char *, va_list));
EXPORT int est_get_api_level(void); 
EXPORT const char * est_get_version(void); 
EXPORT void est_enable_backtrace(int enable);
EXPORT EST_ERROR est_set_ex_data(EST_CTX *ctx, void *ex_data);
EXPORT void * est_get_ex_data(EST_CTX *ctx);
EXPORT EST_CTX * est_server_init(unsigned char *ca_chain, int ca_chain_len,
...
EST_CTX * est_proxy_init(unsigned char *ca_chain, int ca_chain_len,
			 ...
			 EST_CERT_FORMAT cert_format,
                         ...
                         char *uid, char *pwd);
#endif
EXPORT EST_ERROR est_destroy(EST_CTX *ctx);
EXPORT EST_ERROR est_server_set_auth_mode(EST_CTX *ctx, EST_HTTP_AUTH_MODE amode);
EXPORT char *est_server_generate_auth_digest(EST_HTTP_AUTH_HDR *ah, char *HA1);
EXPORT EST_ERROR est_server_start(EST_CTX *ctx);
EXPORT EST_ERROR est_server_stop(EST_CTX *ctx);
EXPORT EST_ERROR est_server_enable_srp(EST_CTX *ctx, int (*cb)(SSL *s, int *ad, void *arg));
EXPORT EST_ERROR est_server_enable_pop(EST_CTX *ctx);
EXPORT EST_ERROR est_server_disable_pop(EST_CTX *ctx);
EXPORT EST_ERROR est_server_handle_request(EST_CTX *ctx, int fd);
EXPORT EST_ERROR est_server_set_dh_parms(EST_CTX *ctx, DH *dh);
EXPORT EST_ERROR est_server_init_csrattrs(EST_CTX *ctx, char *csrattrs, int crsattrs_len);
EXPORT EST_ERROR est_server_set_retry_period(EST_CTX *ctx, int seconds);
EXPORT EST_ERROR est_server_set_ecdhe_curve(EST_CTX *ctx, int nid);
 ...
 */
EXPORT EST_ERROR est_set_ca_enroll_cb(EST_CTX *ctx, int (*cb)(unsigned char * pkcs10, 
	                       int p10_len, unsigned char **pkcs7, int *pkcs7_len, 
			       char *user_id, X509 *peer_cert, void *ex_data));
EXPORT EST_ERROR est_set_ca_reenroll_cb(EST_CTX *ctx, int (*cb)(unsigned char * pkcs10, 
	                         int p10_len, unsigned char **pkcs7, int *pkcs7_len, 
				 char *user_id, X509 *peer_cert, void *ex_data));
EXPORT EST_ERROR est_set_csr_cb(EST_CTX * ctx, unsigned char *(*cb)(int*csr_len, void *ex_data));
EXPORT EST_ERROR est_set_http_auth_cb(EST_CTX * ctx, int (*cb)(EST_CTX*, EST_HTTP_AUTH_HDR*, X509*, void*));
EXPORT EST_ERROR est_set_http_auth_required(EST_CTX * ctx, EST_HTTP_AUTH_REQUIRED required);
EXPORT EST_ERROR est_add_attributes_helper(X509_REQ *req, int nid, void *string, unsigned long chtype);
EXPORT EST_ERROR est_get_attributes_helper(unsigned char **der_ptr, int *der_len, int *new_nid);
EXPORT EST_ERROR est_decode_attributes_helper(char *csrattrs, int csrattrs_len, 
 ...
 */
EXPORT X509_REQ *est_read_x509_request(unsigned char *csr, int csr_len,
	                         EST_CERT_FORMAT csr_format);
EXPORT EVP_PKEY *est_load_key(unsigned char *key, int key_len, int format);
EXPORT int est_convert_p7b64_to_pem(unsigned char *certs_p7, int certs_len, unsigned char **pem);
